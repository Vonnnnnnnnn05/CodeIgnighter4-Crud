...
<body class="bg-light">
    <div class="d-flex">
        ...
        <nav id="sidebar" class="sidebar bg-white border-end">
            ...
            <ul class="nav flex-column px-2 mt-3">
                <li class="nav-item mb-2"><a class="nav-link d-flex align-items-center" href="<?= base_url('/dashboard') ?>"><i class="bi bi-speedometer2 me-2"></i> Product Dashboard</a></li>
                <li class="nav-item mb-2"><a class="nav-link d-flex align-items-center" href="<?= base_url('/product_card') ?>"><i class="bi bi-upc-scan me-2"></i> Products with Barcode</a></li>
                <li class="nav-item mb-2"><a class="nav-link d-flex align-items-center active" href="<?= base_url('/reports') ?>"><i class="bi bi-bar-chart-line me-2"></i> Reports</a></li>
                <li class="nav-item mt-4"><a href="<?= base_url('/logout') ?>" class="nav-link text-danger d-flex align-items-center logout-pointer logout-link"><i class="bi bi-box-arrow-right me-2"></i> Logout</a></li>
            </ul>
        </nav>

        <!-- Main Content -->
        <div class="main-content flex-grow-1 p-4">
            <button id="toggleSidebar" class="btn btn-outline-secondary toggle-btn"><i class="bi bi-chevron-left"></i></button>

            <div class="container-fluid">
                <h2 class="mb-3">Reports & Analytics</h2>

                <?php
                // Prepare aggregate data with safe fallbacks
                ...
                // Calculate average price and today's created count
                $priceSum = 0.0;
                $priceCount = 0;
                if (!empty($productsList)) {
                    foreach ($productsList as $p) {
                        // price may be string or null
                ...
                // Last 6 months labels and counts
                $monthlyLabels = [];
                $monthlyData = [];
                for ($i = 5; $i >= 0; $i--) {
                ...
                // Top products by price (descending)
                ...
                <!-- Summary cards -->
                <div class="row cards-row g-3 mb-4">
                    ...
                    <div class="col-lg-12">
                        <div class="card shadow-sm">
                            <div class="card-body">
                                ...
                                <div>
                                    <button id="downloadTopCsv" class="btn btn-sm btn-outline-dark"><i class="bi bi-download"></i> Download CSV</button>
                                </div>
                            </div>
    ...
    <script>
        // Prepare data for charts (server -> client)
    ...
    <script>
        // Prepare topProducts data from server
        ...
        function convertToCSV(items) {
            ...
            items.forEach(it => {
                // prefer product_name, fallback to name
                const name = (it.product_name ?? it.name ?? '').toString().replace(/\"/g, '""');
                const price = parseFloat(it.price ?? 0).toFixed(2);
                // Escape double quotes and wrap fields that contain commas/quotes/newlines
    ...
    <script>
        // Sidebar toggle persistence (same behavior as other views)
        document.addEventListener("DOMContentLoaded", function() {
            ...
            toggleBtn.addEventListener("click", () => {
                ...
                try {
                    localStorage.setItem('si_crud_sidebar_collapsed', String(isCollapsed));
                } catch (e) {}
            });
            try {
        ...
        // Logout confirmation using SweetAlert2 - fixed redirect
        document.addEventListener('DOMContentLoaded', function() {
